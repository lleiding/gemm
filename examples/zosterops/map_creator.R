...
loadData = function(el_file=elevation_file, fo_file=forest_file, ha_file=habitat_file)
{
    ## Read in the data
    ...
    if (!all(c(dim(elevation_data) == dim(forest_data), dim(forest_data) == dim(habitat_data)))) {
    ...
    return(c(elevation_data, forest_data, habitat_data))
}
...
convertMap = function(elevation, forest, habitat, run_length=1000, out=map_output_file)
{
    ...
    map_text = c("## ZOSTEROPS EXPERIMENT MAP", "",
                 ...
                 "# Sources: USGS LP-DAAC GFCC30TC (forest cover), CGIAR-CSI SRTM 90m DEM (elevation)",
                 "", "# <id> <x> <y> <elevation> <forest> <habitat>")
...
run = function()
{
    ...
    convertMap(elevation, forest, habitat)
}
